...
public class DataProvider
{
    ...
    public async Task WriteRaw(string filePath, object data)
    {
        var path = Path.Combine(DataDirectory, filePath);
        Directory.CreateDirectory(Path.GetDirectoryName(path)!);
        await using var fs = new FileStream(path, FileMode.Create, FileAccess.Write, FileShare.None);
        await JsonSerializer.SerializeAsync(fs, data, RawJsonOptions);
        logger.LogInformation($"Saved {path}");
    }

    public async Task WriteRaw(string filePath, Stream stream)
    {
        var path = Path.Combine(DataDirectory, filePath);
        Directory.CreateDirectory(Path.GetDirectoryName(path)!);
        await using var fs = new FileStream(path, FileMode.Create, FileAccess.Write, FileShare.None);
    ...
    public async Task<TResult> ReadRaw<TResult>(string filePath)
        where TResult : class
    {
        var path = Path.Combine(DataDirectory, filePath);
        if (!File.Exists(path)) throw new SidekickException($"The data file does not exist. {filePath}");
        ...
        await using var fileStream = File.OpenRead(path);
        return await JsonSerializer.DeserializeAsync<TResult>(fileStream, RawJsonOptions)
               ?? throw new SidekickException("The data file could not be read successfully.");
    }
}
