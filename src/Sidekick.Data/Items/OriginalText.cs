...
public class OriginalText
{
    ...
    public OriginalText(string text)
    {
        ...
        Blocks = Text.Split(SeparatorPattern, StringSplitOptions.RemoveEmptyEntries).Select((x, blockIndex) => new OriginalBlock(x.Trim('\r', '\n'), blockIndex)).ToList();
    }

    /// <summary>
    /// Item sections seperated by dashes when copying an item in-game.
    /// </summary>
    public List<OriginalBlock> Blocks { get; }
