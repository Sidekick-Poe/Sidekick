...
using Sidekick.Apis.Poe.Pseudo.Definitions;
        ...
        */
        ...
        public List<PseudoModifier> Parse(List<ModifierLine> lines)
        {
            ...
            foreach (var definition in Definitions)
            {
                ...
                foreach (var definitionModifier in definition.Modifiers)
                {
                    foreach (var line in lines)
                    {
                        ...
                        {
                            if (modifier.Id != definitionModifier.Id)
                            {
                                continue;
                            }

                            hasModifier = true;
                            result.Modifiers.Add(modifier.Id, definitionModifier.Multiplier);
                        }

                        if (hasModifier)
                        {
                            result.Value += line.Values.Average() * definitionModifier.Multiplier;
                        }
                    }
                }
