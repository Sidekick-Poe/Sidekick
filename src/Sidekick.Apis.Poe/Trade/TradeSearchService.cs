...
) : ITradeSearchService
{
    public async Task<TradeSearchResult<string>> Search(Item item, PropertyFilters? propertyFilters = null, IEnumerable<ModifierFilter>? modifierFilters = null, IEnumerable<PseudoModifierFilter>? pseudoFilters = null)
    {
        try
        {
            ...
            var json = new QueryRequest() { Query = query, }.ToJson(SerializationOptions.WithEnumConverterOptions);
            ...
            var result = await content.FromJsonToAsync<TradeSearchResult<string>?>(SerializationOptions.WithEnumConverterOptions);
            if (result != null)
            {
                return result;
            }
    ...
    public async Task<List<TradeItem>> GetResults(GameType game, string queryId, List<string> ids)
    {
        try
        {
            ...
            var content = await response.Content.ReadAsStreamAsync();
            var result = await content.FromJsonToAsync<FetchResult<Result?>>(SerializationOptions.CamelCaseNaming);
            if (result == null)
            {
                return [];
            }
