...
public class ApiItems1Tests(NinjaTestFixture fixture)
{
    ...
    public void ShockNovaOfProcession()
    {
        ...
        fixture.AssertApiItem(items[0], "shock-nova-of-procession-20-20c");
    }

    [Fact]
    public void FoulbornGloves()
    {
        var items = JsonSerializer.Deserialize<List<ApiItem>>(Data);
        Assert.NotNull(items);
        Assert.Equal(4, items.Count);
        Assert.Equal(4, items[1].MaxLinks);
        fixture.AssertApiItem(items[1], "foulborn-voidbringer-corrupted-chaos-conjurer-gloves");
    }
    ...
    public void LargeCluster()
    {
        ...
        Assert.NotNull(items);
        Assert.Equal(4, items.Count);
        fixture.AssertApiItem(items[2], "12-increased-chaos-damage-8-passives-75");
    }
    ...
    public void SmallCluster()
    {
        ...
        Assert.NotNull(items);
        Assert.Equal(4, items.Count);
        fixture.AssertApiItem(items[3], "6-increased-mana-reservation-efficiency-of-skills-3-passives-75");
    }
}
